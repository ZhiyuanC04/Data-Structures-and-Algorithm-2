...
# Lorelai_start, Lorelai_end = map(int, file_data[2].split())
        ...
        while Queue:
            ...
            if Luke == Luke_end and Lorelai == Lorelai_end:
                Luke_Final, Lorelai_Final = [], []
                while (Luke, Lorelai) != (Luke_start, Lorelai_start):
                    # print(Luke, Lorelai)
                    # print(Luke_start, Lorelai_start)
            ...
            for Next_Luke in AdjacentGraph[Luke] + [Luke]:
                print("luke", Next_Luke)

                for Next_Lorelai in AdjacentGraph[Lorelai] + [Lorelai]:
                    print("lore", Next_Lorelai)
                    # Lorelai's possible moves
                    if Next_Luke != Next_Lorelai and \
                            Next_Luke not in AdjacentGraph[Next_Lorelai] and \
                            Next_Lorelai not in AdjacentGraph[Next_Luke] and \
                            (Next_Luke, Next_Lorelai) not in Visited:
                        # Check if next move is valid
                        Visited.add((Next_Luke, Next_Lorelai))
    ...
    lorelai_start = int(file_data[2].split()[0])
    lorelai_end = int(file_data[2].split()[1])
    queue = [(0, luke_start, lorelai_start)]
    priorvertex = {(luke_start, lorelai_start): None}
    visited = []
    finallen = 0
    luke_path = []
    lorelai_path = []
    while queue:
        print(queue)
        position, luke, lorelai = queue.pop(0)
        if luke == luke_end and lorelai == lorelai_end:
        for lukenext in graph[luke] + [luke]:
            for lorelainext in graph[lorelai] + [lorelai]:
                if (lukenext, lorelainext) not in visited:
                    if lukenext != lorelainext:
                        if lukenext not in graph[lorelainext] and lorelainext not in graph[lukenext]:
                            visited.append((lukenext, lorelainext))
                            queue.append((positon + 1, lukenext, lorelainext))
                            priorvertex[(lukenext, lorelainext)] = (luke, lorelai)
